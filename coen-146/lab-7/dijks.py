...
# https://www.youtube.com/watch?v=VetBkjcm9Go&ab_channel=NeuralNine
import sys
import heapq  # heap
...
class Vertex:
    def __init__(self, node):
        ...
        self.adjacent = (
            {}
        )  # a dictionary holding neighboring vertex labels and weights (e.g., {'u': 6, 'v': 7})
...
class Graph:
    def __init__(self):
        self.vert_dict = (
            {}
        )  # this dictionary keeps node labels and objects e.g., {'u': object_u, 'v': object_v}
        self.num_vertices = 0
    ...
    while len(unvisited_queue):
        ...
        for next in current.adjacent:
            ...
            if new_dist < prev_dist:
                ...
                print(
                    "updated : current = %s next = %s prev_dist = %s new_dist = %s"
                    % (current.get_id(), next.get_id(), prev_dist, new_dist)
                )
            else:
                print(
                    "not updated : current = %s next = %s prev_dist = %s new_dist = %s"
                    % (current.get_id(), next.get_id(), prev_dist, new_dist)
                )
        # Rebuild heap
        # 1. Pop every item
        ...
        unvisited_queue = [
            (v.get_distance(), id(v), v) for v in aGraph if not v.visited
        ]
        heapq.heapify(unvisited_queue)
...
if __name__ == "__main__":
    ...
    print(G.edges())
    # G.add_edges_from(SPT_edges, edge_color = 'r')
    pos = nx.spring_layout(G)
    nx.draw_networkx(G, pos=pos, with_labels=True)
    nx.draw_networkx_edges(G, pos=pos, edgelist=list(SPT_edges), edge_color="r")
    plt.axis("off")
    plt.show()
