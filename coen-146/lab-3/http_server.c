...
int main (int argc, char *argv[]) {
   ...
   serv_addr.sin_port = htons (atoi(argv[1]));dest_file = fopen(net_buff, "w");
   ...
   } else if (option == 2) {                                            // The requested file is returned
   ...
   while (1) {
       ...
       if ( option == 1 ){
        ...
        // read the output of the command
            command_buff[COMMAND_BUFF]; // buffer to hold the result of command execution 

           if (system_info == NULL) { // Check if the command was executed successfully
            ...
            while (fgets(command_buff, sizeof(command_buff), system_info) != NULL) { // Read the result of the command execution 

                // STUDENT WORK 

                printf("%s", command_buff); // Print the result of the command execution
            }
